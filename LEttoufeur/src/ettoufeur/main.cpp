...
int main(int argc, TCHAR* argv[]) {
	...
	while ((opt = getopt(argc, argv, _T("hukln:p:s:d:x:"))) != -1)
	{
		switch (opt)
		{
		case 'n':
		{
			...
			bRet = GetProcessPIDFromName(optarg, &dwPid);
			if (!bRet)
				return PrintInputError(INPUT_ERROR_NONEXISTENT_PID);
			else
				szProcessName = optarg;
			break;
		}
		case 'p':
		{
			isUsingProcessPID = TRUE;
			char* buf = C::Convert::StringToString(optarg);
			dwPid = atoi(buf);
			if (!verifyPID(dwPid))
				return PrintInputError(INPUT_ERROR_NONEXISTENT_PID);
		...
		case 's':
		{
			...
			wcscpy_s(szServiceName, _countof(szServiceName), optarg);
			break;
		}
		case 'd':
		{
			...
			memset(szDriverPath, 0, sizeof(szDriverPath));
			wcscpy_s(szDriverPath, _countof(szDriverPath), optarg);
			break;
		}
		case 'x':
		{
	...
	{
		const char* buf = C::Convert::StringToString(szHandleToClose);
		char log[128];
		long lVal = strtol(buf, 0, 16);
		sprintf(log,"Closing Handle : 0x%x\n", lVal);
		Info(log);
		ProcExpKillHandle(dwPid, lVal);
